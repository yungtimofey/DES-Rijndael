...
class RijndaelTest {
    ...
    void testSBox() throws WrongIrreduciblePolynomialException {
        ...
        byte[] SBox =  Rijndael.getSBox(283);

        for (int i = 0; i < sBox.length; i++) {
            int s = GaloisFieldPolynomialsCalculator.convertByteToInt(SBox[i]);
            if ( sBox[i] != s) {
                assert (false);
                return;
            }
    ...
    @Test
    void testRCON() throws WrongIrreduciblePolynomialException {
        int[] rcon = new int[]{
    ...
    @Test
    void testReverseSBox() throws WrongIrreduciblePolynomialException {
        int[] reverse = {
        ...
        byte[] reversedSBox = Rijndael.getInverseSBox(283);

        for (int i = 0; i < reverse.length; i++) {
            int translated = GaloisFieldPolynomialsCalculator.convertByteToInt(reversedSBox[i]);
            if (translated != reverse[i]) {
                assert (false);
            }
        }
        assert (true);
